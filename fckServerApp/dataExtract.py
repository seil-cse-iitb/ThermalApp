...
def getSeatData(i):
    ...
    con = mysql.connect()
    cur = con.cursor()
    query = "select temperature from temperature_analysis where node_id = '%d' order by timestamp desc limit 1" %(i)
    cur.execute(query)
    res = cur.fetchone()
    con.close()
    #print res[0]
...
def makeTempMatrix(temp):
    ...
    tempMatrix[4][0]=temp[3]
    tempMatrix[4][7]=temp[2]
    tempMatrix[4][12]=temp[1]
    tempMatrix[4][17]=temp[0]

    tempMatrix[3][1]=temp[20]
    tempMatrix[3][3]=temp[19]
    tempMatrix[3][7]=temp[18]
    tempMatrix[3][11]=temp[17]
    tempMatrix[3][15]=temp[16]
    tempMatrix[3][20]=temp[6]
    tempMatrix[3][28]=temp[5]
    tempMatrix[3][34]=temp[4]

    tempMatrix[2][0]=temp[23]
    tempMatrix[2][4]=temp[22]
    tempMatrix[2][11]=temp[21]
    tempMatrix[2][15]=temp[9]
    tempMatrix[2][21]=temp[8]
    tempMatrix[2][26]=temp[7]

    tempMatrix[1][0]=temp[26]
    tempMatrix[1][5]=temp[25]
    tempMatrix[1][10]=temp[24]
    tempMatrix[1][14]=temp[12]
    tempMatrix[1][19]=temp[11]
    tempMatrix[1][23]=temp[10]



    tempMatrix[0][1]=temp[28]
    tempMatrix[0][6]=temp[27]
    tempMatrix[0][8]=temp[15]
    tempMatrix[0][13]=temp[14]
    tempMatrix[0][17]=temp[13]
    #print(tempMatrix)
    '''
    k=0
...
def storeFeedbac():
    if(request.method == 'POST'):
        ...
        print (a[0],a[1])
        query = "insert into userFeedback(seatNumber,PMVRating,timestamp,roomName) values('%d','%d','%s','%s')" %(int(a[1]),int(a[0]),datetime.now(),'LH')
        cur.execute(query)
	con.commit()
        #print(a)
    return redirect("http://10.129.149.7/tc/seating.html")
